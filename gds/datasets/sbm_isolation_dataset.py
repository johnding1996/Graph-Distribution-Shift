...
class SBMIsolationDataset(GDSDataset):
    ...
    def __init__(self, version=None, root_dir='data', download=False, split_scheme='official', random_split=False,
        ...
        if self.subgraph:
            ...
            subgraph = GSN(dataset_name='SBM-Isolation', dataset_group='SBM', induced=True, id_type=self.id_type,
                           k=self.k)

            self.graphs_ptg, self.encoder_ids, self.d_id, self.d_degree = subgraph.preprocess('data/SBM-Isolation')
