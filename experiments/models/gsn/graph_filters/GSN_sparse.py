...
class GSN_sparse(nn.Module):
    def __init__(self,
        ...
        if self.msg_kind == 'gin':
            ...
            if self.id_scope == 'local':
                ...
                self.central_node_id_encoder = central_encoder(kwargs['id_embedding'], d_id,
                                                               extend=kwargs['extend_dims'])
    ...
    def forward(self, x, edge_index, **kwargs):
        ...
        if self.msg_kind == 'gin':
            ...
            out = self.update_fn(
                torch.cat((x, self.propagate(edge_index=edge_index, x=x, identifiers=identifiers)), -1))
